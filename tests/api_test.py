...
try:
    ...
    r = c.post_json("/api/register", email="alice@example.com", password="secret123")
    check(
        "JSON register -> envelope with assigned ID",
        r.status == 200
        and r.json()["ok"] is True
        and r.json()["id"] == "100010"
    ...
    for i in range(12):
        ...
        r = c.post_json(
            "/api/future",
            title=f"API pagination seed {i} mk{stamp}",
            body=f"filler body {i}",
            category=cat,
        )
        shorts.append(r.json()["short"])
    check("12 seeds created via JSON", all(len(s) == 5 for s in shorts) and len(set(shorts)) == 12)
    ...
    check("list: past-the-end page is empty, not an error", r["items"] == [] and r["page"] == 99)
    r = c.get("/api/futures?per_page=1000").json()
    check("list: per_page clamped to 100", r["per_page"] == 100 and len(r["items"]) == 12)
    ...
    check("LIKE wildcards escaped (no match-all)", r["total"] == 0, f"total={r['total']}")
    ...
    check("cat=Cities -> 6", r["total"] == 6 and all(i["category"] == "Cities" for i in r["items"]))
    ...
    r = c.post(
        ...
        future_id=target,
        type="people",
        title="Maya the Navigator",
        body="Form post from the site UI",
    )
    check("form POST contribute -> 302", r.status == 302 and r.location == f"/f/{target}")
    r = c.post_json(
        "/api/contribute",
        future_id=target,
        type="place",
        title="Harbor Steps",
        body="JSON post from app.js",
    )
    check(
        "JSON POST contribute -> envelope",
        r.status == 200 and r.json()["ok"] is True and r.json()["redirect"] == f"/f/{target}",
    )
    r = c.post("/api/future", title="", body="x")
    check(
        "validation error is JSON even for form posts",
        r.status == 400 and r.json()["error"] == "Title required",
    )
    r = c.request("POST", "/api/future", data={"title": "x"}, send_csrf=False)
    check("missing CSRF -> 403 JSON", r.status == 403 and "CSRF" in r.json()["error"])
    anon = Client(server)
    anon.prime("/api/health")
    r = anon.post_json("/api/future", title="x", body="y")
    check(
        "anonymous JSON write -> 401", r.status == 401 and r.json()["error"] == "Sign in required"
    )
    ...
    check("contributions filtered by type", r["total"] == 1 and r["items"][0]["type"] == "people")
    r = c.get("/api/futures/ZZZZZ")
    check("unknown seed -> 404 JSON", r.status == 404 and r.json()["ok"] is False)
    ...
    check(
        "second account registered with the next ID",
        r.status == 200 and r.json()["ok"] and r.json()["id"] == "100012",
    )
    bob_id = r.json()["id"]

    r = c.post_json(
        f"/api/future/{target}/edit",
        title=f"Renamed by alice {stamp}",
        body="updated body",
        category="Work",
    )
    check("owner edits seed -> 200", r.status == 200 and r.json()["redirect"] == f"/f/{target}")
    r = c.get(f"/api/futures/{target}").json()
    check(
        "edit persisted + edited_at set",
        r["future"]["title"] == f"Renamed by alice {stamp}" and r["future"]["edited_at"],
    )
    ...
    check("non-owner seed edit -> 403", r.status == 403 and "creator" in r.json()["error"])

    r = b.post_json(
        "/api/contribute", future_id=target, type="story", title="Bob's story", body="from bob"
    )
    ...
    r = b.post_json(
        f"/api/contribution/{bob_contrib['id']}/edit", title="Bob's story v2", body="edited by bob"
    )
    check("author edits own contribution", r.status == 200 and r.json()["ok"])
    r = c.post_json(f"/api/contribution/{bob_contrib['id']}/edit", title="hijack", body="nope")
    ...
    check(
        "comments carry parent_id (null at top level)",
        alice_cm["parent_id"] is None and bob_cm["parent_id"] is None,
    )
    r = c.post_json(f"/api/comment/{alice_cm['id']}/edit", body="Alice's comment, edited")
    ...
    check("contribution + comment show (edited)", "(edited)" in c.get(f"/f/{target}").text)

    # -- 9. Branch flow ------------------------------------------------------------
    r = c.post_json(
        "/api/future",
        title=f"Branch parent {stamp}",
        body="root of the branch test",
        category="Life",
    )
    parent = r.json()["short"]
    r = b.post_json(
        "/api/future", title=f"Child branch {stamp}", body="branched off", parent=parent
    )
    child = r.json()["short"]
    ...
    check(
        "parent lists child in child_branches",
        [x["short_id"] for x in r["child_branches"]] == [child],
    )
    check(
        "child page shows Branched from",
        "Branched from" in b.get(f"/f/{child}").text and parent in b.get(f"/f/{child}").text,
    )
    check(
        "parent page shows Recent Branches with child",
    ...
    check(
        "alice notified of bob's contribution",
        ("contribute", "contributed a Story: Bob's story") in kinds,
        f"{kinds}",
    )
    check(
        "alice notified of bob's comment",
        any(t == "comment" and "commented on your future" in x for t, x in kinds),
    )
    check(
        "alice notified of bob's branch",
        any(t == "branch" and "branched off your future" in x for t, x in kinds),
    )
    ...
    r = c.post_json(
        "/api/comment", future_id=target, body="alice replies to bob", parent_id=bob_cm["id"]
    )
    check("reply accepted", r.status == 200)
    replies = c.get(f"/api/futures/{target}/comments").json()["items"]
    check(
        "reply stored with parent_id",
        any(x["parent_id"] == bob_cm["id"] and x["author"] == alice_id for x in replies),
    )
    bn = b.get("/api/notifications").json()
    check(
        "bob notified of the reply",
        any(n["type"] == "reply" and "replied to your comment" in n["text"] for n in bn["items"]),
    )
    # alice is both the replier and the seed creator -> nothing for herself
    an = c.get("/api/notifications").json()
    check(
        "no self-notification for the creator's reply",
    ...
    r = b.post_json("/api/comment", future_id=target, body="bob self-reply", parent_id=bob_cm["id"])
    ...
    check("unread drops to 0 after read", b.get("/api/notifications").json()["unread"] == 0)

    # -- 11. Input caps ---------------------------------------------------------------
    r = c.post_json("/api/future", title="x" * 501, body="y")
    check("501-char title -> 400", r.status == 400 and "500" in r.json()["error"])
    r = c.post_json("/api/contribute", future_id=target, type="people", title="y" * 201, body="y")
    check("201-char contribution title -> 400", r.status == 400 and "200" in r.json()["error"])
    r = c.post_json("/api/comment", future_id=target, body="z" * 5001)
    check("5001-char comment -> 400", r.status == 400 and "5000" in r.json()["error"])
    ...
    check(
        "user endpoint: stats",
        r["ok"]
        and r["registered"] is True
        and r["stats"]["futures"] >= 14
        and r["stats"]["contributions"] >= 1,
    )
    check("user endpoint: recent futures listed", len(r["recent_futures"]) >= 10)
    r = c.get("/api/users/whoami_not_a_user")
    check("unknown user -> 404", r.status == 404)
    r = c.get("/api/feed?limit=10").json()
    check(
        "feed returns items with kind",
        len(r["items"]) == 10 and all("kind" in x for x in r["items"]),
    )
    ...
    check("referrer policy header", h.get("referrer-policy") == "strict-origin-when-cross-origin")
    ...
    for i in range(130):
        ...
        if r.status == 429:
            limited_at = i + 1
            break
    check("POST flood gets 429", limited_at is not None, "never limited in 130 posts")
    if limited_at:
        check("429 body is JSON", r.json()["ok"] is False and "slow down" in r.json()["error"])
    check("GETs still served while POST-limited", c.get("/api/health").status == 200)
